...
const options = {
    ...
    onClose (selectedDates) {
        ...
        if (selectedDate > Date.now()) btStart.removeAttribute("disabled");
        else (0, _notiflixNotifyAio.Notify).failure("Please choose a date in the future");
    }
};
const calendar = (0, _flatpickrDefault.default)(inputTime, options);
...
function startCountdown(ms) {
    ...
    countdownInterval = setInterval(()=>{
        const { days: days1 , hours: hours1 , minutes: minutes1 , seconds: seconds1  } = convertMs(ms);
        timeDays.textContent = addLeadingZero(days1);
        timeHours.textContent = addLeadingZero(hours1);
        timeMinutes.textContent = addLeadingZero(minutes1);
        timeSeconds.textContent = addLeadingZero(seconds1);
        if (ms <= 0) {
...
function convertMs(ms) {
    ...
    const day = hour * 24;
    const days2 = Math.floor(ms / day);
    const hours2 = Math.floor(ms % day / hour);
    const minutes2 = Math.floor(ms % day % hour / minute);
    const seconds2 = Math.floor(ms % day % hour % minute / second);
    return {
        days: days2,
        hours: hours2,
        minutes: minutes2,
